...
app_configdir="$swiz_configdir/${app_name^}audio"
app_group="media"
app_port="87989"
app_reqs=("curl" "sqlite3")
app_servicefile="$app_nameaudio.service"
app_dir="/opt/${app_name^}"
app_binary="${app_name^}"
#Remove any dashes in appname per FS
app_lockname="${app_name//-/}audio"
app_branch="nightly"
#ToDo: Update branch
...
_nginx_readarr() {
    if [[ -f /install/.nginx.lock ]]; then
        ...
        bash /usr/local/bin/swizzin/nginx/"$app_name"audio.sh
...
echo_success "${app_name^}audio installed"
